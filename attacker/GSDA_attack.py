...
from utils.gsda_utils import eig_vector
...
class gsda_attack(nn.Module):
    ...
    def _forward_step(
        ...
        output_curr_iter = self.model(self.defense_head(input_curr_iter.transpose(1, 2)))
        ...
        if self.uniform_loss_weight != 0:
            ...
            constrain_loss = constrain_loss + self.uniform_loss_weight * uniform
                ...
                with torch.no_grad():
                    for k in range(b):
                        if input_curr_iter.size(1) < input_all.size(1):
                            ...
                            batch_k_adv_output = self.model(self.defense_head(batch_k_pc.transpose(1, 2)))
                            attack_success[k] = (
                                _compare(
                            ...
                            adv_output = self.model(self.defense_head(input_all[k:k+1].transpose(1, 2)))
    ...
    #                 response1.requires_grad_()
    #                 if self.optim == "adam":
    #                     optimizer = optim.Adam([factor, response, response1], lr=self.lr)
    #                 elif self.optim == "sgd":
    #                     optimizer = optim.SGD([factor, response, response1], lr=self.lr)
    #                 else:
    #                     assert False, "Not support such optimizer."
    ...
    #                 reg_loss_l2 = attacker.get_loss(
    #                     input_curr_iter.permute(0, 2, 1),
    ...
    #                 reg_loss_curv = attacker.get_loss(
    #                     input_curr_iter.permute(0, 2, 1),
    #                     pc_ori.permute(0, 2, 1),
    #                     normal_ori.permute(0, 2, 1),
    #                     "curv",
    #                 )
    #                 reg_loss[:, 3] = reg_loss_curv

    #             if (
    #                 logits.argmax(1).item() != target.item()
    #                 and (reg_loss <= output_reg).all()
    #             ):
    #                 output_points = input_curr_iter.permute(0, 2, 1).detach()
    #                 output_reg = reg_loss

    #         else:
    #             f, output_points, output_reg, initial_lr = (
    #                 attacker.gsda_boundary_projection_2(
    #                     pc_ori.permute(0, 2, 1),
    #                     x_,
    #                     v,
    #                     factor,
